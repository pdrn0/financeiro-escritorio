...
app.post('/api/transacoes', (req, res) => {
  ...
  const id = run(
    ...
    [tipo, categoria, descricao, parseFloat(valor), data, mes, ano, pago_por]
  );
  const row = queryOne(
    'SELECT t.*,c.cor FROM transacoes t LEFT JOIN categorias c ON t.categoria=c.nome WHERE t.id=?',
    [id]
  );
  res.json(row);
});
...
app.delete('/api/transacoes/:id', (req, res) => {
  run('DELETE FROM transacoes WHERE id=?', [+req.params.id]);
  res.json({ ok: true });
});

app.get('/api/categorias', (req, res) => {
  const { tipo } = req.query;
  if (tipo) res.json(queryAll('SELECT * FROM categorias WHERE tipo=? ORDER BY id', [tipo]));
  else res.json(queryAll('SELECT * FROM categorias ORDER BY tipo, id'));
});
...
app.get('/api/resumo', (req, res) => {
  ...
  const detalhesIgor = queryAll(`SELECT t.categoria, t.tipo, t.descricao, t.valor, t.data, c.cor FROM transacoes t LEFT JOIN categorias c ON t.categoria=c.nome ${where ? where + ' AND' : 'WHERE'} t.pago_por='igor' ORDER BY t.data DESC`, p);
  const detalhesPedro = queryAll(`SELECT t.categoria, t.tipo, t.descricao, t.valor, t.data, c.cor FROM transacoes t LEFT JOIN categorias c ON t.categoria=c.nome ${where ? where + ' AND' : 'WHERE'} t.pago_por='pedro' ORDER BY t.data DESC`, p);

  res.json({ entradas: totalEntradas, saidas: totalSaidas, saldo: totalEntradas - totalSaidas, porCategoria, porMes, relatorio: { igor: { pagouSaidas: igPagouSaidas, pagouEntradas: igPagouEntradas, cotaSaidas: cotaSaidasCada, cotaEntradas: cotaEntradasIgor, creditoSaidas: igCreditoSaidas, saldoLiquido: saldoLiquidoIgor, detalhes: detalhesIgor }, pedro: { pagouSaidas: pedPagouSaidas, pagouEntradas: pedPagouEntradas, cotaSaidas: cotaSaidasCada, cotaEntradas: cotaEntradasPedro, creditoSaidas: pedCreditoSaidas, saldoLiquido: saldoLiquidoPedro, detalhes: detalhesPedro }, dividaPedroPagaIgor, dividaIgorPagaPedro } });
});
