...
db.exec(`
  CREATE TABLE IF NOT EXISTS transacoes (
    ...
    valor REAL NOT NULL, data TEXT NOT NULL, mes INTEGER NOT NULL,
    ano INTEGER NOT NULL, pago_por TEXT NOT NULL DEFAULT 'igor',
    criado_em TEXT DEFAULT (datetime('now'))
  );
  CREATE TABLE IF NOT EXISTS categorias (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    nome TEXT NOT NULL, tipo TEXT NOT NULL, cor TEXT DEFAULT '#888888'
  );
...
app.post('/api/transacoes', (req, res) => {
  ...
  res.json(db.prepare('SELECT t.*,c.cor FROM transacoes t LEFT JOIN categorias c ON t.categoria=c.nome WHERE t.id=?').get(result.lastInsertRowid));
});
...
app.get('/api/categorias', (req, res) => {
  ...
  res.json(tipo ? db.prepare('SELECT * FROM categorias WHERE tipo=? ORDER BY id').all(tipo) : db.prepare('SELECT * FROM categorias ORDER BY tipo, id').all());
});

app.post('/api/categorias', (req, res) => {
  const { nome, tipo, cor } = req.body;
  const r = db.prepare('INSERT INTO categorias (nome,tipo,cor) VALUES (?,?,?)').run(nome, tipo, cor||'#888888');
  res.json({ id: r.lastInsertRowid, nome, tipo, cor });
});
...
app.get('/api/resumo', (req, res) => {
  ...
  res.json({
    entradas: totalEntradas, saidas: totalSaidas, saldo: totalEntradas - totalSaidas,
    porCategoria, porMes,
    relatorio: {
